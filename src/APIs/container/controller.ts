...
export default {
    ...
    try {
      ...
      res.status(201).json({ message: 'Container Booked Successfully' , data : container_data });
    } catch (error : any) {
      console.log(error)
  ...
  payment_container : async(req : Request , res : Response) => {
         try{
            ...
            res.status(200).json({ message: 'Clien Payment Secret' ,  data : paymentverify?.client_secret});
         } 
         catch(e : any){
  ...
  booking_container_tracking : async( req : Request , res : Response) => {
    try {
      ...
      let a =  await service.update_book_conatiner_tracking(req.body , id);
        if(a){
          res.status(200).json({ message: 'Container Tracking Updated  Successfully'});
        }
  ...
  get_all_client_booking :  async( req : Request , res : Response) => {
    try{
          if(req){}
          let a = await service.get_container_details();
