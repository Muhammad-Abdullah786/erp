...
export default {
  ...
  booking_container: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Call the service to save the booking and create payment
      const { container, payment } =
        await service.save_book_container_with_payment(body);

      logger.info("Container booked and payment order created successfully", {
